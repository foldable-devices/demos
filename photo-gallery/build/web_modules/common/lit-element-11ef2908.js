import{i as N,m as z,T as A,r as B,N as O,a as P}from"./lit-html-ec56d68b.js";/**
 * @license
 ...
 * subject to an additional IP rights grant found at
 * http://polymer.github.io/PATENTS.txt
 */function D(a){let b=o.get(a.type);b===void 0&&(b={stringsArray:new WeakMap(),keyString:new Map()},o.set(a.type,b));let c=b.stringsArray.get(a.strings);if(c!==void 0)return c;const d=a.strings.join(z);return c=b.keyString.get(d),c===void 0&&(c=new A(a,a.getTemplateElement()),b.keyString.set(d,c)),b.stringsArray.set(a.strings,c),c}const o=new Map();/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 * http://polymer.github.io/LICENSE.txt
 ...
 * subject to an additional IP rights grant found at
 * http://polymer.github.io/PATENTS.txt
 */const m=new WeakMap(),S=(a,b,c)=>{let d=m.get(b);d===void 0&&(B(b,b.firstChild),m.set(b,d=new O(Object.assign({templateFactory:D},c))),d.appendInto(b)),d.setValue(a),d.commit()};/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 * http://polymer.github.io/LICENSE.txt
 ...
 * subject to an additional IP rights grant found at
 * http://polymer.github.io/PATENTS.txt
 */const E=(a,b)=>`${a}--${b}`;let p=!0;typeof window.ShadyCSS==="undefined"?p=!1:typeof window.ShadyCSS.prepareTemplateDom==="undefined"&&(console.warn("Incompatible ShadyCSS version detected. Please update to at least @webcomponents/webcomponentsjs@2.0.2 and @webcomponents/shadycss@1.3.1."),p=!1);const T=a=>b=>{const c=E(b.type,a);let d=o.get(c);d===void 0&&(d={stringsArray:new WeakMap(),keyString:new Map()},o.set(c,d));let e=d.stringsArray.get(b.strings);if(e!==void 0)return e;const g=b.strings.join(z);e=d.keyString.get(g);if(e===void 0){const f=b.getTemplateElement();p&&window.ShadyCSS.prepareTemplateDom(f,a),e=new A(b,f),d.keyString.set(g,e)}return d.stringsArray.set(b.strings,e),e},U=["html","svg"],V=a=>{U.forEach(b=>{const c=o.get(E(b,a));c!==void 0&&c.keyString.forEach(d=>{const{element:{content:e}}=d,g=new Set();Array.from(e.querySelectorAll("style")).forEach(f=>{g.add(f)}),C(d,g)})})},F=new Set(),W=(a,b,c)=>{F.add(a);const d=c?c.element:document.createElement("template"),e=b.querySelectorAll("style"),{length:g}=e;if(g===0){window.ShadyCSS.prepareTemplateStyles(d,a);return}const f=document.createElement("style");for(let i=0;i<g;i++){const k=e[i];k.parentNode.removeChild(k),f.textContent+=k.textContent}V(a);const h=d.content;c?R(c,f,h.firstChild):h.insertBefore(f,h.firstChild),window.ShadyCSS.prepareTemplateStyles(d,a);const j=h.querySelector("style");if(window.ShadyCSS.nativeShadow&&j!==null)b.insertBefore(j.cloneNode(!0),b.firstChild);else if(c){h.insertBefore(f,h.firstChild);const i=new Set();i.add(f),C(c,i)}},X=(a,b,c)=>{if(!c||typeof c!=="object"||!c.scopeName)throw new Error("The `scopeName` option is required.");const d=c.scopeName,e=m.has(b),g=p&&b.nodeType===11&&!!b.host,f=g&&!F.has(d),h=f?document.createDocumentFragment():b;S(a,h,Object.assign({templateFactory:T(d)},c));if(f){const j=m.get(h);m.delete(h);const i=j.value instanceof P?j.value.template:void 0;W(d,h,i),B(b,b.firstChild),b.appendChild(h),m.set(b,j)}!e&&g&&window.ShadyCSS.styleElement(b.host)};/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 * http://polymer.github.io/LICENSE.txt
...
*/const J="adoptedStyleSheets"in Document.prototype&&"replace"in CSSStyleSheet.prototype,K=Symbol();class L{constructor(a,b){if(b!==K)throw new Error("CSSResult is not constructable. Use `unsafeCSS` or `css` instead.");this.cssText=a}get styleSheet(){return this._styleSheet===void 0&&(J?(this._styleSheet=new CSSStyleSheet(),this._styleSheet.replaceSync(this.cssText)):this._styleSheet=null),this._styleSheet}toString(){return this.cssText}}const Y=a=>{if(a instanceof L)return a.cssText;{if(typeof a==="number")return a;throw new Error(`Value passed to 'css' function must be a 'css' function result: ${a}. Use 'unsafeCSS' to pass non-literal values, but
            take care to ensure page security.`)}},Z=(a,...b)=>{const c=b.reduce((d,e,g)=>d+Y(e)+a[g+1],a[0]);return new L(c,K)};/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 * http://polymer.github.io/LICENSE.txt
