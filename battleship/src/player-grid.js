...
export class PlayerGrid extends GameGrid {
  ...
  isCellABoatPreviouslyShot(cell) {
    ...
    return (cell.shot && cell.type != undefined &&
            cell.type === this.grid[this._previousShot.x][this._previousShot.y].type);
  }
  ...
  getNeighborCell(direction, cell) {
    ...
    switch(direction) {
      case GridDirection.Left:
        candidateCell = {x: cell.x, y: Math.max(cell.y - 1, 1)};
        break;
      case GridDirection.Right:
        candidateCell = {x: cell.x, y: Math.min(cell.y + 1, 10)};
        break;
      case GridDirection.Top:
        candidateCell = {x: Math.max(cell.x - 1, 1), y: cell.y};
        break;
      case GridDirection.Bottom:
        candidateCell = {x: Math.min(cell.x + 1, 10), y: cell.y};
        break;
      default:
        console.error("wrong direction")
        return;
  ...
  walkGrid(direction, cell) {
    ...
    // We're trying to climb but we found a water that we shot previously so
    // we should climb the other direction.
    if (this.isCellAPreviouslyMissedShot(this.grid[candidateCell.x][candidateCell.y]))
      return this.walkGrid(this.oppositeDirection(direction), candidateCell);
  ...
  enemyShoot() {
    ...
    if (this._previousShot.x != 0 && this._previousShot.y != 0) {
      ...
      if (hasLeft)
        leftCell = {x: this._previousShot.x, y: this._previousShot.y - 1};
      ...
      if (hasRight)
        rightCell = {x: this._previousShot.x, y: this._previousShot.y + 1};
      ...
      if (hasTop)
        topCell = {x: this._previousShot.x - 1, y: this._previousShot.y}
      ...
      if (hasBottom)
        bottomCell = {x: this._previousShot.x + 1, y: this._previousShot.y}

      let tryVertical = Math.random() >= 0.5;
      // If the cells around were shot and had a boat we need to climb up the grid.
      if ((hasLeft && this.isCellABoatPreviouslyShot(this.grid[leftCell.x][leftCell.y])) ||
          (hasRight && this.isCellABoatPreviouslyShot(this.grid[rightCell.x][rightCell.y]))) {
      ...
      if ((hasTop && this.isCellABoatPreviouslyShot(this.grid[topCell.x][topCell.y])) ||
          (hasBottom && this.isCellABoatPreviouslyShot(this.grid[bottomCell.x][bottomCell.y]))) {
      ...
      if (tryVertical && !bottomCandidate && !topCandidate)
        tryVertical = false;
      if (!tryVertical && !leftCandidate && !rightCandidate)
        tryVertical = true;
    ...
    if(this.isShip(this.grid[x][y])) {
      ...
      // By chance we found another boat around. Let's remember it so we can come back to it.
      ...
      this._previousShot = {x: x, y: y, type: ship.type};
      if (ship.destroyed) {
