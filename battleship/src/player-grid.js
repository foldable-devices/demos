...
export class PlayerGrid extends GameGrid {
  ...
  walkGridToFindEmptyCell(direction, cell) {
    ...
    // If we shot there then bail out, we can't shoot there.
  ...
  walkGridToFindCandidateToShoot(direction, cell, maxWalk, currentSpaceNumber) {
    ...
    if (this.isCellABoatPreviouslyShot(this.grid[nextCell.x][nextCell.y])) {
      // We continue to walk.
      return this.walkGridToFindCandidateToShoot(direction,
          nextCell, maxWalk, ++currentSpaceNumber)
    }
  ...
  isBoatHorizontal(cell) {
    ...
    if (!this.isCellAtTheEge(GridDirection.Left, cell)) {
      ...
      if(this.isCellABoatPreviouslyShot(this.grid[leftCell.x][leftCell.y]))
        isHorizontal = true;
    }

    if (!this.isCellAtTheEge(GridDirection.Right, cell)) {
      let rightCell = this.getNeighborCell(GridDirection.Right, cell);
      if(this.isCellABoatPreviouslyShot(this.grid[rightCell.x][rightCell.y]))
        isHorizontal = true;
    }
    return isHorizontal;
  }
  ...
  isBoatVertical(cell) {
    ...
    if (!this.isCellAtTheEge(GridDirection.Top, cell)) {
      ...
      if(this.isCellABoatPreviouslyShot(this.grid[topCell.x][topCell.y]))
        isVertical = true;
    }

    if (!this.isCellAtTheEge(GridDirection.Bottom, cell)) {
      let bottomCell = this.getNeighborCell(GridDirection.Bottom, cell);
      if(this.isCellABoatPreviouslyShot(this.grid[bottomCell.x][bottomCell.y]))
        isVertical = true;
    }
    return isVertical;
  }
  ...
  enemyShoot() {
    ...
    if (this._previousShot.x != 0 && this._previousShot.y != 0) {
      ...
      // try random.
      if (boatIsVertical === null && boatIsHorizontal === null)
          tryVertical = Math.random() >= 0.5;
      else if (boatIsHorizontal)
        tryVertical = false;
      else if (boatIsVertical)
        tryVertical = true;
